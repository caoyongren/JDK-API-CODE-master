    ...
    implements Map<K,V>, Cloneable, Serializable
{
    ...
    final int hash(Object k) {
        ...
        //Step 1：调用Key自带的hashCode()函数获得初始哈希值
        h ^= k.hashCode();
     ...
     * Step 2:
     *   没有使用　除法散列法　％(取模)，　使用　＆
     * HashMap中链表的length总是2的n次方，length总是2的n次方时，
     * h & (length-1)运算等价于对length取模，也就是h%length，但&具有更高的效率．
     *
     * 2的n次方，必然是偶数，length -1 为奇数，转换成二进制之后，最后一位为1
     * 当h & (length-1)时，最后一位的是0还是1，就取决于h的值，也就是说index的奇偶性取决于h
     */
